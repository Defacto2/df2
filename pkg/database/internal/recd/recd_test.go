...
func TestValid(t *testing.T) {
	...
	newset, offset := []byte("2006-01-02T15:04:06Z"), []byte("2006-01-02T15:04:05Z")
	b, err = recd.Valid(newset, newset)
	assert.Nil(t, err)
	assert.True(t, b)
	b, err = recd.Valid(newset, offset)
	assert.Nil(t, err)
	assert.True(t, b)
	newset, offset = []byte("2026-01-02T15:04:06Z"), []byte("2006-01-02T15:04:05Z")
	b, err = recd.Valid(newset, offset)
	assert.Nil(t, err)
	assert.False(t, b)
	b, err = recd.Valid(offset, newset)
	assert.Nil(t, err)
	assert.False(t, b)
}
